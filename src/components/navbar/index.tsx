...
const Navbar = ({ 
  ...
  hovered,
  ...props }: {
  onOpenSidenav: (event: React.MouseEvent<HTMLSpanElement, MouseEvent>) => void;
  mini: boolean;z
  hovered: boolean;
  brandText: string;
  ...
  // const { onOpenSidenav, brandText, mini, hovered } = x
  const [darkmode, setDarkmode] = React.useState(
    document.body.classList.contains('dark'),
  );

 
  function onOpenSidenav(event: React.MouseEvent<HTMLSpanElement, MouseEvent>): void {
    // Implement the logic to open the sidenav here
  }
  return (
    <nav
      className={`duration-175 linear w-full  right-3 top-3 flex flex-row flex-wrap items-center justify-between rounded-xl bg-white/30 transition-all    p-2 backdrop-blur-xl dark:bg-[#0b14374d] md:right-[30px] md:top-4 xl:top-[20px]`}
    >
      ...
      <div className="relative mt-[3px] flex h-[61px] w-[355px] flex-grow items-center justify-around gap-2 rounded-full bg-white px-2 py-2 shadow-xl shadow-shadow-500 dark:!bg-navy-800 dark:shadow-none md:w-[365px] md:flex-grow-0 md:gap-1 xl:w-[365px] xl:gap-2">
        <div className="flex h-full items-center rounded-full !bg-foreground text-navy-700 dark:bg-navy-900 dark:text-white xl:w-[225px]">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="block h-full w-full rounded-full !bg-foreground text-sm font-medium text-navy-700 outline-none  sm:w-fit"
          />
        </div>
        <span
