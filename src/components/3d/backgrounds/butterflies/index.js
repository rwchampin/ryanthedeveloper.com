import { Color, DoubleSide, HalfFloatType, InstancedBufferAttribute, InstancedMesh, Math } from 'three'
...
const { randFloat: rnd, randFloatSpread: rndFS } = Math;
...
export default function Butterflies(params) {
  ...
  function initTextures(texturePosition, textureVelocity) {
    ...
    for (let k = 0, kl = posArray.length; k < kl; k += 4) {
      dummy.set(rndFS(1.0), rndFS(1.0), rndFS(1.0)).normalize().multiplyScalar(rndFS(config.attractionRadius1 * 2.0)).toArray(posArray, k);
      posArray[k + 3.0] = rnd(0.1, 1.0);

      dummy.set(rndFS(1.0), rndFS(1.0), rndFS(1.0)).normalize().multiplyScalar(rndFS(0.5)).toArray(velArray, k);
      velArray[k + 3.0] = 0.0;
    }
  }
}
